    ...
    width: 16px;
        ...
        .message-meta-actions {
            ...
            transition: var(--transition);
            z-index: 2;
            /* 没展开的时候（opacity:0）也不能让它挡在上面偷点击——
               不然点旁边贴着的语音条播放键，很容易被这个看不见的收藏/删除键截胡 */
            pointer-events: none;
        }
        ...
        /* 触屏设备：靠 JS 判断"轻点"而不是靠 :hover（滑动/滚动时手指一碰到消息气泡
           就会触发 :hover，容易在滑屏时误触发回复/收藏/删除这几个按钮），
           点一下消息气泡才展开，滑动不会触发 */
        .message-content-wrapper.show-actions-tap .message-meta-actions {
            opacity: 1;
            transform: translateY(0);
            pointer-events: auto;
        }
    ...
    background-color: rgba(255, 71, 87, 0.1);
        ...
        .coin-btn, .batch-btn, .continue-btn, .attachment-btn, .poke-btn, .combo-btn, .sticker-send-btn {
            background-color: var(--toolbar-btn-bg, var(--message-received-bg));
            color: var(--toolbar-btn-color, var(--text-secondary));
        }
        .attachment-btn:hover, .continue-btn:hover, .batch-btn:hover, .coin-btn:hover, .poke-btn:hover, .combo-btn:hover, .sticker-send-btn:hover {
            color: var(--text-primary);
        }
 ...
 .mood-kaomoji-span {
    ...
    to { opacity:1; transform: translateY(0) scale(1); }
        ...
        /* 表情包发送键：平时用原来左边的表情按钮（combo-btn）就够了，
           只有在"底部栏收纳"模式下、combo-btn 被收起来之后，才需要它顶上 */
        .sticker-send-btn { display: none !important; }
        body.bottom-collapse-mode .sticker-send-btn { display: flex !important; }
...
/* ============================================================
   聊天表情包预览区（输入框上方，可与文字配套或单独发送）
   ============================================================ */
#chat-sticker-preview {
    animation: stickerPreviewSlideIn 0.2s ease;
}
@keyframes stickerPreviewSlideIn {
...
/* ──────── 语音气泡（伪语音消息，无 AI 配音） ──────── */
.message-wrapper.has-voice .message-content-wrapper,
.message-wrapper.has-voice .message {
    -webkit-user-select: none;
    user-select: none;
...
/* 输入框允许多行显示（最多约3行，超出滚动）
   原 CSS 强制 nowrap + hidden + ellipsis 锁成单行，这里覆盖回 textarea 默认行为，
   让字多了之后正常往下换行，而不是被裁切/省略号截断 */
.message-input {
...
/* ============================================================
   ...
   ============================================================ */
:root {
  --transfer-main: #4db8a6;
  --transfer-dark: #2d9980;
  --transfer-light: #e8f8f5;
}

.red-packet-card {
    border-radius: 14px !important;
    overflow: hidden !important;
    box-shadow: 0 4px 18px rgba(45,153,128,0.22), 0 1px 4px rgba(45,153,128,0.10) !important;
    border: none !important;
    width: 240px !important;
}

.red-packet-card .rp-body {
    background: linear-gradient(135deg, #5ec4b0 0%, #2d9980 55%, #1f7a65 100%) !important;
    padding: 14px 16px 16px !important;
    position: relative !important;
}

.red-packet-card .rp-body::before {
    content: '' !important;
...
.red-packet-card .rp-amount-text {
    font-size: 26px !important;
    font-weight: 700 !important;
    letter-spacing: 0.5px !important;
}

.red-packet-card .rp-footer {
    background: var(--secondary-bg, #fff) !important;
    border-top: 1px solid rgba(45,153,128,0.15) !important;
    padding: 9px 14px !important;
}

.red-packet-card.opened .rp-body {
    background: linear-gradient(135deg, #a8c8c2 0%, #8aafaa 100%) !important;
}

.red-packet-btn:hover,
.red-packet-btn:hover i {
    color: var(--transfer-main) !important;
}

/* 底部收纳面板里的转账图标，跟其他 collapsed-extra-btn 保持一致的样式（不需要额外写，
   本身就是复用同一个 class） */
...
/* ---- 情侣空间主界面 ---- */
#couple-space-modal .modal-content {
    max-width: 560px;
    max-height: 92vh;
    padding: 0 !important;
    overflow: hidden;
...
.cs-tab:hover { background: var(--primary-bg); color: var(--text-primary); }
.cs-tab.active {
    background: var(--accent-color);
    color: #fff;
    box-shadow: 0 2px 10px rgba(var(--accent-color-rgb), 0.3);
}
.cs-tab i { margin-right: 4px; font-size: 11px; }
...
.cs-feed-action:hover { color: var(--accent-color); }
.cs-feed-action.liked { color: var(--accent-color); }
.cs-feed-action i { font-size: 13px; }
.cs-feed-comments {
    margin-top: 10px;
    padding-left: 8px;
...
.cs-comment-item .cname {
    font-weight: 600;
    color: var(--accent-color);
    flex-shrink: 0;
}
.cs-comment-item .ctext {
    color: var(--text-primary);
    word-break: break-word;
}
.cs-comment-item .cactions {
    margin-left: auto;
    display: flex;
    gap: 6px;
    flex-shrink: 0;
}
.cs-comment-item .cactions button {
    background: none;
    border: none;
    font-size: 10px;
    color: var(--text-secondary);
...
.cs-comment-item .cactions button:hover { color: var(--accent-color); }
.cs-comment-input-wrap {
    display: flex;
    gap: 6px;
    margin-top: 8px;
}
.cs-comment-input-wrap input {
    flex: 1;
    padding: 6px 12px;
    border: 1px solid var(--border-color);
    border-radius: 20px;
...
.cs-comment-input-wrap input:focus {
    border-color: var(--accent-color);
}
.cs-comment-input-wrap button {
    padding: 6px 14px;
    border-radius: 20px;
    border: none;
    background: var(--accent-color);
...
.cs-letter-item:hover {
    border-color: rgba(var(--accent-color-rgb), 0.3);
}
.cs-letter-item .letter-header {
    display: flex;
    justify-content: space-between;
    align-items: center;
    margin-bottom: 6px;
...
.cs-letter-item .letter-from {
    font-weight: 600;
    color: var(--accent-color);
}
.cs-letter-item .letter-time {
    opacity: 0.6;
}
.cs-letter-item .letter-body {
    font-size: 14px;
    line-height: 1.8;
    white-space: pre-wrap;
    word-break: break-word;
    font-family: var(--letter-font, var(--font-family));
}
.cs-letter-item .letter-reply {
    margin-top: 10px;
    padding: 10px 14px;
    background: rgba(var(--accent-color-rgb), 0.06);
    border-radius: 12px;
...
.cs-letter-item .letter-reply .reply-label {
    font-size: 10px;
    font-weight: 700;
    color: var(--accent-color);
    letter-spacing: 1px;
...
.cs-letter-item:hover .letter-delete {
    opacity: 0.5;
}
.cs-letter-item:hover .letter-delete:hover {
    opacity: 1;
    color: #ff4757;
}
...
.cs-letter-editor textarea:focus {
    border-color: var(--accent-color);
}
.cs-letter-editor .editor-toolbar {
    display: flex;
    gap: 8px;
    margin-top: 10px;
    flex-wrap: wrap;
...
.cs-letter-editor .editor-toolbar label {
    font-size: 11px;
    color: var(--text-secondary);
    display: flex;
    align-items: center;
...
.cs-letter-editor .editor-toolbar input[type="text"] {
    width: 120px;
    padding: 4px 8px;
    border: 1px solid var(--border-color);
    border-radius: 6px;
...
.cs-letter-editor .editor-toolbar input[type="text"]:focus {
    border-color: var(--accent-color);
}
.cs-letter-editor .editor-actions {
    display: flex;
    gap: 8px;
    margin-top: 10px;
}
.cs-letter-editor .editor-actions button {
    padding: 8px 20px;
    border-radius: 12px;
    border: none;
    font-size: 13px;
...
.cs-letter-editor .editor-actions .btn-send {
    background: var(--accent-color);
    color: #fff;
    flex: 1;
}
.cs-letter-editor .editor-actions .btn-send:hover {
    box-shadow: 0 4px 16px rgba(var(--accent-color-rgb), 0.3);
}
.cs-letter-editor .editor-actions .btn-cancel {
    background: var(--primary-bg);
    color: var(--text-secondary);
}
.cs-letter-editor .editor-actions .btn-cancel:hover {
    background: var(--border-color);
}
...
.cs-bill-item .bill-icon {
    font-size: 20px;
    width: 36px;
    text-align: center;
    flex-shrink: 0;
}
.cs-bill-item .bill-info {
    flex: 1;
    min-width: 0;
}
.cs-bill-item .bill-amount {
    font-weight: 700;
    font-size: 15px;
}
.cs-bill-item .bill-amount.income { color: #4CAF50; }
.cs-bill-item .bill-amount.expense { color: #ff4757; }
.cs-bill-item .bill-desc {
    font-size: 12px;
    color: var(--text-secondary);
    margin-top: 2px;
}
.cs-bill-item .bill-note {
    font-size: 11px;
    color: var(--accent-color);
    font-style: italic;
    max-width: 120px;
...
.cs-bill-item:hover .bill-delete { opacity: 0.8; }
.cs-bill-item .bill-delete:hover { color: #ff4757; opacity: 1; }
...
.cs-bill-summary .stat {
    flex: 1;
    text-align: center;
}
.cs-bill-summary .stat .num {
    font-size: 20px;
    font-weight: 700;
}
.cs-bill-summary .stat .num.income { color: #4CAF50; }
.cs-bill-summary .stat .num.expense { color: #ff4757; }
.cs-bill-summary .stat .label {
    font-size: 10px;
    color: var(--text-secondary);
    margin-top: 2px;
}
...
.cs-bill-editor input,
.cs-bill-editor select {
    padding: 8px 12px;
    border: 1px solid var(--border-color);
    border-radius: 10px;
    font-size: 13px;
...
.cs-bill-editor input:focus,
.cs-bill-editor select:focus {
    border-color: var(--accent-color);
}
.cs-bill-editor .bill-amount-input {
    width: 100px;
}
.cs-bill-editor .bill-desc-input {
    flex: 1;
    min-width: 120px;
}
.cs-bill-editor .bill-add-btn {
    padding: 8px 18px;
    border-radius: 10px;
    border: none;
    background: var(--accent-color);
...
.cs-bill-editor .bill-add-btn:hover {
...
.cs-diary-item .diary-title {
    font-weight: 600;
    color: var(--text-primary);
    font-size: 14px;
}
.cs-diary-item .diary-body {
    font-size: 13px;
    line-height: 1.7;
    color: var(--text-primary);
    white-space: pre-wrap;
...
.cs-diary-item .diary-reply .reply-name {
    font-weight: 600;
    color: var(--accent-color);
}
.cs-diary-item .diary-delete {
    background: none;
    border: none;
    color: var(--text-secondary);
    cursor: pointer;
...
.cs-diary-item:hover .diary-delete { opacity: 0.7; }
.cs-diary-item .diary-delete:hover { color: #ff4757; opacity: 1; }
...
.cs-gift-item .gift-icon {
    font-size: 28px;
    width: 44px;
    text-align: center;
    flex-shrink: 0;
}
.cs-gift-item .gift-info {
    flex: 1;
}
.cs-gift-item .gift-name {
    font-weight: 600;
    font-size: 14px;
}
.cs-gift-item .gift-from {
    font-size: 11px;
    color: var(--text-secondary);
}
.cs-gift-item .gift-note {
    font-size: 11px;
    color: var(--accent-color);
    font-style: italic;
    margin-top: 2px;
}
.cs-gift-item .gift-affection {
    font-size: 12px;
    color: #ff6b6b;
    font-weight: 700;
    padding: 2px 10px;
...
.cs-gift-shop .gift-card:hover {
    transform: translateY(-3px);
    border-color: var(--accent-color);
    box-shadow: 0 6px 18px rgba(var(--accent-color-rgb), 0.12);
}
.cs-gift-shop .gift-card .gift-icon-big {
    font-size: 32px;
    display: block;
    margin-bottom: 4px;
}
.cs-gift-shop .gift-card .gift-name-sm {
    font-size: 12px;
    font-weight: 600;
}
.cs-gift-shop .gift-card .gift-price {
    font-size: 11px;
    color: var(--text-secondary);
}
.cs-gift-shop .gift-card .gift-aff {
    font-size: 10px;
    color: #ff6b6b;
    font-weight: 700;
}
...
.cs-affection-bar .level {
    font-size: 22px;
    font-weight: 800;
    color: var(--accent-color);
}
.cs-affection-bar .level-label {
    font-size: 14px;
    font-weight: 600;
    color: var(--text-primary);
    margin-left: 8px;
}
.cs-affection-bar .aff-num {
    font-size: 16px;
    color: var(--text-secondary);
    margin-top: 4px;
}
.cs-affection-bar .aff-progress {
    height: 6px;
    background: var(--border-color);
    border-radius: 3px;
    margin-top: 8px;
...
.cs-affection-bar .aff-progress .fill {
    height: 100%;
    border-radius: 3px;
...
@media (max-width: 480px) {
    ...
    .cs-tabs { padding: 8px 12px 0; gap: 2px; }
    .cs-tab { font-size: 11px; padding: 6px 10px; }
    .cs-bill-editor { flex-direction: column; }
    .cs-bill-editor .bill-amount-input { width: 100%; }
    .cs-gift-shop { grid-template-columns: repeat(auto-fill, minmax(80px, 1fr)); }
}
