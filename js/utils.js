...
// HTML 转义工具（红包/问卷等功能会用到，milk-main 之前一直没有这个全局函数，
// 导致相关代码一用就直接报错 "escapeHtml is not defined"）
...
async function applyCustomFont(url, silent) {
    if (!url || !url.trim()) {
    ...
    try {
        ...
        // 页面刚打开、自动恢复上次字体这种"后台操作"失败了，不弹醒目的错误提示打扰用户，
        // 只在控制台留痕；只有用户自己主动点"上传字体/应用字体链接"时才弹出错误提示
        if (!silent) showNotification('字体加载失败，请检查链接是否有效', 'error');
        throw e;
    }
}
